              ...
              <div class="flex gap-3 bg-[#797DFC1A] lg:p-10 p-2 rounded-lg w-full">
                <div class="relative">
                  <img
                    ...
                    alt=""
                  />
                  <div id="activeStatus" class="h-3 w-3 absolute right-1 top-2 ${element.isActive ? "bg-green-500": "bg-red-500"} rounded-full"></div>
                </div>
                <div class="space-y-2">
                  <span class=" font-semibold pr-4">#<span>${element.category}</span></span>
                  <span class=" font-semibold">Author : ${element.author.name}</span>
                  ...
                  <div class="flex justify-between">
                    <div class="flex gap-6">
                      ...
                      <h3 class="font-bold space-x-2 text-lg opacity-70">
                        <i class="fa-regular fa-eye"></i><span>${element.view_count}</span>
                      </h3>
                      <h3 class="font-bold space-x-2 text-lg opacity-70">
...
const markAsread=(title,view)=>{
    ...
    const div=document.createElement('div');
    div.className="flex gap-3 justify-between shadow-md lg:p-8 p-2 bg-white mb-3 rounded-md"
    div.innerHTML=`
...
const latestPost=async()=>{
    ...
    data.forEach((item)=>{
        const div=document.createElement('div')
        div.innerHTML=`
        <div class="card  bg-base-100 shadow-xl p-6 border-2 border-gray-100">
        <figure class="rounded-3xl mb-4"><img  src="${item.cover_image}" alt="post" /></figure>
        <div class="card-body p-0 space-y-1">
          <h3 class="space-x-2 opacity-70"><i class="fa-regular fa-calendar-days"></i><span>${item.author?.posted_date||"No publish date"}</span></h3>
          <h2 class="text-xl font-bold">${item.title}</h2>
          <p>${item.description}</p>
          <div class="flex gap-2">
            <img class="h-12 w-12 border-2 border-sky-200 rounded-full" src="${item.profile_image}" alt="">
            <div>
